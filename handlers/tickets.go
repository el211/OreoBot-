...
func handleTicketSetup(s *discordgo.Session, i *discordgo.InteractionCreate, opts []*discordgo.ApplicationCommandInteractionDataOption) {
	...
	if err := gs.Save(); err != nil {
		slog.Warn("Failed to save guild state after ticket setup", "guild", i.GuildID, "error", err)
	}
...
func handleTicketAddCategory(s *discordgo.Session, i *discordgo.InteractionCreate, opts []*discordgo.ApplicationCommandInteractionDataOption) {
	...
	gs.Unlock()
	if err := gs.Save(); err != nil {
		slog.Warn("Failed to save guild state", "error", err)
	}
...
func handleTicketAddSubcategory(s *discordgo.Session, i *discordgo.InteractionCreate, opts []*discordgo.ApplicationCommandInteractionDataOption) {
	...
	if !found {
		for _, c := range getCfg().Tickets.Categories {
			if c.ID == catID {
...
func parseComponentEmoji(emoji string) *discordgo.ComponentEmoji {
	...
	// If it matches, extract name and ID for a valid ComponentEmoji.
	s := emoji
	animated := false
	...
	// Reject bare custom-emoji names like ":mc:" or "mc" with no Unicode value —
	// Discord requires an ID for custom emojis.  Only pass through real Unicode emoji.
	// A heuristic: if the string has no multi-byte rune (i.e. it's pure ASCII and
	// not a Unicode emoji), skip it to avoid a 400 from Discord.
	for _, r := range emoji {
		if r > 127 {
			// Contains at least one non-ASCII rune → treat as a Unicode emoji.
			return &discordgo.ComponentEmoji{Name: emoji}
		}
	}
	// Pure ASCII → likely a bare custom emoji name without ID; skip it.
	return nil
}
