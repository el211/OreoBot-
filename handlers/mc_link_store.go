...
func InitMCLinkStore() {
	backend := getCfg().Minecraft.LinkBackend
	if backend == "" {
		backend = "file"
	}
	...
	case "mongodb":
		...
		if err != nil {
			slog.Error("Failed to initialise MongoDB link store", "error", err)
			os.Exit(1)
		}
		MCStore = s
		slog.Info("MC link backend: mongodb")
	default:
		MCStore = newFileLinkStore()
		slog.Info("MC link backend: file")
	}
}
...
func newMongoLinkStore() (*mongoLinkStore, error) {
	uri := getCfg().Database.MongoDB.URI
	db := getCfg().Database.MongoDB.Database
	if uri == "" || db == "" {
