...
func RegisterNoPing(s *discordgo.Session, cfg *config.Config) {
	...
	bypass := make(map[string]bool, len(cfg.NoPing.BypassRoles))
	for _, id := range cfg.NoPing.BypassRoles {
		bypass[strings.TrimSpace(id)] = true
	}

	s.AddHandler(func(s *discordgo.Session, m *discordgo.MessageCreate) {
		handleNoPing(s, m, &cfg.NoPing, protected, bypass)
	})

	slog.Info("NoPing active", "protected_roles", len(protected), "bypass_roles", len(bypass))
}

func handleNoPingCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	if !isAdmin(s, i) {
