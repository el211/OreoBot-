...
func handleWarn(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if storage.DB != nil {
		if err := storage.DB.AddWarning(i.GuildID, target.ID, w); err != nil {
			slog.Warn("Failed to add warning to DB", "guild", i.GuildID, "user", target.ID, "error", err)
		}
	}
	...
	if err := gs.Save(); err != nil {
		slog.Warn("Failed to save guild state after warn", "guild", i.GuildID, "error", err)
	}
...
func handleClearWarnings(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if storage.DB != nil {
		if err := storage.DB.ClearWarnings(i.GuildID, target.ID); err != nil {
			slog.Warn("Failed to clear warnings in DB", "guild", i.GuildID, "user", target.ID, "error", err)
		}
	}
	...
	gs.Unlock()
	if err := gs.Save(); err != nil {
		slog.Warn("Failed to save guild state after clear warnings", "guild", i.GuildID, "error", err)
	}
...
func handlePurge(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if len(ids) == 1 {
		if err := s.ChannelMessageDelete(i.ChannelID, ids[0]); err != nil {
			slog.Warn("Failed to delete message", "channel", i.ChannelID, "error", err)
		}
	} else {
		if err := s.ChannelMessagesBulkDelete(i.ChannelID, ids); err != nil {
			slog.Warn("Failed to bulk delete messages", "channel", i.ChannelID, "error", err)
		}
	}
...
func handleModlog(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	gs.Unlock()
	if err := gs.Save(); err != nil {
		slog.Warn("Failed to save guild state after modlog set", "guild", i.GuildID, "error", err)
	}
...
func logModAction(s *discordgo.Session, guildID, action string, target, moderator *discordgo.User, reason, duration string) {
	if storage.DB != nil {
		if err := storage.DB.AddModCase(guildID, storage.ModCase{
			GuildID:   guildID,
			UserID:    target.ID,
			ModID:     moderator.ID,
			Action:    action,
			...
			slog.Warn("Failed to add mod case to DB", "guild", guildID, "error", err)
		}
	}

	gs := storage.GetGuild(guildID)
	logCh := config.EffectiveModLogChannel(getCfg(), gs)
	if logCh == "" {
		return
	}
