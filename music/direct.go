...
func (d *DirectBackend) Play(vc *discordgo.VoiceConnection, song *Song, volume int, done chan<- struct{}) {
	...
	if streamURL == "" {
		...
		if err != nil {
			slog.Error("Failed to resolve song", "error", err)
			return
		}
	...
	if err != nil {
		slog.Error("ffmpeg stdout pipe error", "error", err)
		return
	}
	...
	if err := ffmpegCmd.Start(); err != nil {
		slog.Error("ffmpeg start error", "error", err)
		return
	}
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				slog.Info("Stream ended (ffmpeg output EOF)")
			} else {
