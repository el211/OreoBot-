...
func (s *SQLiteDB) Init() error {
	if err := os.MkdirAll(filepath.Dir(s.Path), 0755); err != nil {
		slog.Warn("Failed to create database directory", "path", filepath.Dir(s.Path), "error", err)
	}
	...
	slog.Info("SQLite initialised", "path", s.Path)
	return nil
}
...
func (s *SQLiteDB) ClearWarnings(guildID, userID string) error {
	ctx, cancel := dbCtx()
	defer cancel()
	_, err := s.db.ExecContext(ctx, "DELETE FROM warnings WHERE guild_id = ? AND user_id = ?", guildID, userID)
	return err
}
...
func (m *MongoDB) Init() error {
	...
	if err := os.MkdirAll(m.dir, 0755); err != nil {
		return fmt.Errorf("failed to create mongodb fallback dir: %w", err)
	}
	slog.Info("MongoDB fallback (JSON files) initialised", "path", m.dir)
	slog.Info("For production, replace storage/database.go MongoDB section with the official mongo-driver")
	return nil
}
