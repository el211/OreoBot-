...
func (svc *Service) runPolling() {
	slog.Info("Payment polling started — checking every 60s")
...
func (svc *Service) pollGuild(gs *config.GuildState) {
	...
	for _, inv := range snapshot {
		...
		if err := gs.Save(); err != nil {
			slog.Warn("Failed to save guild state after marking invoice paid", "invoice", inv.Number, "error", err)
		}
...
func (svc *Service) checkPaid(inv config.CommissionInvoice) (bool, string) {
	...
	if svc.paypal != nil && inv.PayPalInvoiceID != "" &&
		...
		if err != nil {
			slog.Warn("PayPal poll error", "invoice", inv.Number, "error", err)
	...
	if svc.stripe != nil && inv.StripeSessionID != "" &&
		...
		if err != nil {
			slog.Warn("Stripe poll error", "invoice", inv.Number, "error", err)
		} else if status == "paid" {
	...
	if svc.coinbase != nil && inv.CoinbaseChargeID != "" &&
		...
		if err != nil {
			slog.Warn("Coinbase poll error", "invoice", inv.Number, "error", err)
